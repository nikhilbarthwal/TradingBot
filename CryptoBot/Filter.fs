...
module Filter =
    ...
    let private transform (length, window) (row: Array<Fraction>) =
    ...
    type private ComputeState(maxOrder: int, length: int, window: int) =
        ...
        let v = Array.Create window (fun i -> Fraction(window - 1 - 2 * i, 2))
        ...
        let input, output: MatrixVar<Fraction> * MatrixVar<Fraction> =
            ...
            let gen (i, j) = Array.Convolve window (v.Row(i), v.Row(j))
        ...
        member this.Matrix(order: int): MatrixConst<float> =
            let gen k = Array.Convolve (order + 1) (output.Row(0), v.Column(k))
            Array.Create window gen |> transform (length, window)
